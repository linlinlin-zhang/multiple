...
- [ ] **SEL-01**: 双击卡片选中，持久蓝色细线包围，对话框绑定到选中卡片，再次双击取消选中
- [ ] **SEL-02**: 无选中卡片时对话框禁用，尝试输入显示提示弹窗
- [ ] **SEL-03**: 卡片右上角悬停显示删除按钮，初始卡片和延伸卡片不可删除
- [ ] **SEL-04**: 双击卡片名称可编辑，Enter/失焦保存，Esc取消
- [ ] **DGEN-01**: 对话框所有操作以选中卡片为上下文锚点
- [ ] **DGEN-02**: 选中卡片后可在对话框输入约束直接生成新的成图方向子节点
- [ ] **DGEN-03**: 发送按钮左侧有 thinking/no-thinking 模式切换，影响本次请求
- [ ] **RMOD-01**: 原分析按钮改为"研究"悬停下拉菜单
- [ ] **RMOD-02**: 下拉菜单含"分析"(no-thinking)和"探索"(thinking)选项，悬停显示气泡说明
- [ ] **RMOD-03**: 探索模式返回创作方向同时搜集相关资料存储在节点数据中
- [ ] **RMOD-04**: 双击含资料的方向卡片弹出资料弹窗展示搜集到的资源
- [ ] **ISH-01**: 图片弹窗下方有圆形分享按钮
- [ ] **ISH-02**: 点击生成独立的单图分享网页
- [ ] **ISH-03**: 分享网页包含图片、AI讲解、原图摘要、生成方向说明
...
**Goal:** 将画布升级为深度交互式工作台，用户通过选中卡片与系统对话，选择快速分析或深度探索模式，自定义卡片信息，并直接控制生成方向。

**Target features:**
- 卡片选中与交互：双击选中/取消、删除、重命名
- 对话框重构：绑定选中卡片、直接生成方向、思考模式切换
- 研究模式：分析(no-thinking) vs 探索(thinking+资料搜集)
- 单图分享：图片弹窗内一键生成分享网页
