...
- **HE-P2-05 complex deterministic regression layer:** Extended the deterministic canvas action runner with deep action-field checks, array-length assertions, required/forbidden text patterns, and trace-stage checks; expanded `scripts/evals/canvas-action-complex-regression.jsonl` to 107 fixtures and wired it into `npm run test:guards`.
- **HE-P2-06 intent reliability hardening:** Tightened direction fallback so existing-direction image generation remains `generate_image`, erroneous comparison cards are not kept beside recovered direction cards, mechanical/duplicated direction-card tool calls are repaired into distinct usable directions, negated delete/search/direction requests do not authorize those actions, and product/code phrases like `产品定位` / `移动平均` no longer trigger workspace routing.
- **HE-P2-07 MiMo live eval checkpoint:** Ran the 40-case MiMo live suite, fixed/retested the prior multi-file matrix and history direction-card failures, and recorded residual live-model volatility for later policy-level work instead of continuing one-off patches.
