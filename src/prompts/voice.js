...
export function buildRealtimeInstruction(context) {
  ...
  return [
    ...
    "- Content shapes: note {text,sections}; plan {summary,steps:[{title,description,time,priority}]}; todo {items:[{text,done,priority,rationale}]}; weather {location,forecast,highlights}; map {location,address,points}; table {columns,rows}; timeline {items:[{phase,title,description,time}]}; comparison {items:[{title,summary,pros,cons}]}; metric {metrics:[{label,value,note}]}; quote {quotes:[{text,source}]}; link/web {title,url,description,source,faviconUrl}; code {language,code,explanation,usage}.",
