...
export function buildDeepThinkSystemPrompt(lang) {
  ...
  if (lang === "en") {
    return [
      ...
      "The frontend will turn every card into a canvas node. Create curated source cards for meaningful sources, but never exceed 20 cards total; group low-value duplicate sources into a note/list card.",
      ...
      "Provide as many high-value cards as the task deserves, up to 20 cards total, and 1-20 links.",
      "",
      "# Card Type Guide",
  ...
  return [
    ...
    "前端会把每张卡变成画布节点。为有意义的来源创建精选来源卡，但总数不要超过 20 张；低价值或重复来源应合并进一个笔记/列表卡。",
    "可以按任务需要使用通用图片能力：真实世界视觉证据/参考用公网图片搜索，抽象、创意、推演或概念设计可生成概念图；不要把图片能力绑定到少数示例场景。",
    "根据任务需要输出高价值卡片，最多 20 张，并输出 1-20 条关系。",
    "",
    "# 卡片类型说明",
