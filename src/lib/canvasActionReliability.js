...
const NEGATED_DIRECTION_CARD_RE = /(不要|别|不用|无需|不需要|no|without|do\s+not|don't|dont).{0,18}(先)?(做|创建|生成)?[^，。！？.!?]{0,12}(方向|方向卡|方向卡片|direction|concept)/i;
const MEDIA_FROM_EXISTING_DIRECTION_RE = /(?:根据|基于|按照|用|把|从|选择|选中|current|selected|use|from|based on).{0,24}(方向|方案|概念|direction|option|concept).{0,32}(成图|出图|生成.{0,10}(图片|图像|图|视觉|海报)|generate\s+(?:image|picture|visual))|(?:方向|方案|概念|direction|option|concept).{0,32}(生成.{0,10}(图片|图像|图|视觉|海报)|成图|出图|generate\s+(?:image|picture|visual))/i;
...
function mergePrimaryActions(primary = [], secondary = [], maxActions = 8) {
  ...
  const support = secondary.filter((action) => {
    ...
    return action && !["create_direction", "create_comparison", "generate_image", "generate_video"].includes(type);
  });
...
export function shouldCreateDirectionActions(message = "", reply = "") {
  ...
  if (NEGATED_DIRECTION_CARD_RE.test(requestText)) return false;
