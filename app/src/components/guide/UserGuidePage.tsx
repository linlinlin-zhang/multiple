...
const zhSections: GuideSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { action: "图片编辑界面", detail: "打开生成图后，可重生成、输入修改要求、用“选择”涂抹局部区域、换宽高比或下载。" },
      { action: "分享界面", detail: "点击图片详情里的分享按钮，可给单图命名、生成/复制分享链接，也能重命名或下载。" },
...
const enSections: GuideSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { action: "Image editing", detail: "Open a generated image to regenerate, describe edits, brush a local region, change aspect ratio, or download." },
      { action: "Share panel", detail: "Use the share button in Image Details to name one image, create/copy a link, rename it, or download it." },
