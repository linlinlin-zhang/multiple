...
function MetadataGrid({ rows }: { rows: { label: string; value: string }[] }) {
  return (
    <div className="grid grid-cols-2 gap-x-6 gap-y-2 mt-4">
      {rows.map((row) => (
        <div key={row.label}>
          <div className="text-[13px] font-mono text-cabinet-inkMuted">{row.label}</div>
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto cabinet-scrollbar px-8 pb-12">
      <img
        ...
        alt={title}
        loading="lazy"
        className="max-w-full rounded border border-cabinet-border"
      />
      <MetadataGrid
        rows={[
...
function LinkAssetDetail({ node }: { node: Node }) {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto cabinet-scrollbar px-8 pb-12">
      <div className="flex flex-col items-center justify-center py-12">
        ...
        {url && (
          <a
            ...
            className="text-[15px] text-cabinet-ink mt-4 font-medium hover:underline break-all text-center"
          >
            {url}
          </a>
        )}
      ...
      {description && (
        <div className="mt-4">
          <div className="text-[13px] font-mono text-cabinet-inkMuted">Description</div>
